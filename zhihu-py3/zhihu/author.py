...
class Author(BaseZhihu):
    ...
    def __init__(self, url, name=None, motto=None, follower_num=None,
                 ...
                 thank_num=None, photo_url=None,session=None,**kwargs):
        """创建用户类实例.
    ...
    def _make_card(self):
        if self.card is None and self.url is not None:
            ...
            #https://www.zhihu.com/node/MemberProfileCardV2?params={%22url_token%22:%22wanfeiyy%22}
    ...
    def usr_info(self):
        ...
        tmp = self.soup.find(
            'a', class_='zm-profile-header-user-weibo')
        weibo_url = tmp['href'] if tmp is not None else 'unknown'
        education = self._my_get_info(self.soup,'education')
        location = self._my_get_info(self.soup,'location')
        business = self._my_get_info(self.soup,'business')
        return  {
            'gender':gender,
            'weibo_url':weibo_url,
            'education':education,
    ...
    def _my_get_info(self,html,attr):
        info_dict = {
            'education': "education",
            'location': 'location',
            'business':'business',
        ...
        if attr not in info_dict:
            return  False
        res = html.find('span',class_=info_dict[attr])
        if res is not None:
            return res['title'] if res.has_attr('title')  else 'unknow'
        else:
            return 'unknow'
    ...
    def _follow_ee_ers(self, t, skip=0):
        ...
        #print (request_url,headers,params,data)
        gotten_date_num = 20
        offset = skip
           ...
           # print (json_data,json_data['msg'])
            ...
            for html in json_data['msg']:
                ...
                if author_url is not None:
                    ...
                    member = client.author(author_url).usr_info()
                    weibo_url = member['weibo_url']
